...
export type EventTypes = BaseEventTypes|string;
...
//Note that if nothing is passed, no type needs to be defined, since "void" is the default return
    ...
    export type EventResult = RunPipelineStepSyncResult | void;
...
export function unlistenTo(id: ListenerID, event: EventTypes = null) {
    if (event == null) {
        for (let key in eventListeners) {
            const [evID,evName] = key.split("_")
            if (evID == id){
                delete eventListeners[key]
                if(__debug) console.log(`[Eventbus] Removed listener "${key}"`,eventListeners);
            }
        }
    } else {
        delete eventListeners[`${id}_${event}`]
        if(__debug) console.log(`[Eventbus] Removed listener "${id}" for event "${event}"`,eventListeners);
    }
}
...
export function showToast(msg:string, severity:AlertColor = "success"){
    fireEvent<ToastEventPayload>(BaseEventTypes.ToastEvent,{msg:msg,severity:severity})
}
/**
